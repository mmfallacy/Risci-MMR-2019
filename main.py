...
with open(os.path.join(CURRENTDIR,"config","config.json"),"r") as CONFIG_FILE:
    ...
    THEME_LIST = [i.lower() for i in _temp["THEMES"]]
...
def theme_route_contestant(theme,contestantnum):
    if theme in THEME_LIST and contestantnum <= TOTAL_CONTESTANT_NUM and contestantnum > 0:
        if isRegistered()and not submitStatus():
            return render_template("contestant-page.j2", criteriaList=CRITERIA_LIST, tContestantNum = TOTAL_CONTESTANT_NUM,theme=theme,contestant=CONTESTANT_LIST[contestantnum], contestantnum=str(contestantnum), contestantdata=(CONTESTANT_LIST[contestantnum]["male"],CONTESTANT_LIST[contestantnum]["female"]))  
        else:
            return TEMPLATE_403
    else:
        return TEMPLATE_404
